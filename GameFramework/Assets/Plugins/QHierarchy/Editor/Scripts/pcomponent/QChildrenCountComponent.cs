...
namespace qtools.qhierarchy.pcomponent
{
    public class QChildrenCountComponent: QBaseComponent 
    {
        ...
        {
            int childrenCount = GetChildrenCount(gameObject);
            if (childrenCount > 1) GUI.Label(rect, childrenCount.ToString(), labelStyle);
        }

        private int GetChildrenCount(GameObject gameObject)
        {
            return gameObject.GetComponentsInChildren<Transform>().Length;
